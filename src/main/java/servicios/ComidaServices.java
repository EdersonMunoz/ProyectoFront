...
public class ComidaServices {
	...
	public Comida consultarComida(Integer id)
	{
		...
		objComida = target.request(MediaType.APPLICATION_JSON_TYPE).get(Comida.class);
		return objComida;
	}
	...
	public List<Comida> listarComidas()
	{
		...
		listaComidas = target.request(MediaType.APPLICATION_JSON).get(new GenericType<List<Comida>>() {});		
		return listaComidas;
	}
	...
	public Comida registrarComida(Comida objComidaRegistar)
	{
		Comida objComida=null;
		WebTarget target = client.target(this.endPoint);	    
	    Entity<Comida> data = Entity.entity(objComidaRegistar, MediaType.APPLICATION_JSON_TYPE);
	    objComida = target.request(MediaType.APPLICATION_JSON_TYPE).post(data, Comida.class);		
		return objComida;
	}
	...
	public Comida actualizarComida(Comida objComidaActualizar,Integer id)
	{
		Comida objComida=null;		
		WebTarget target = client.target(this.endPoint+"/"+id);	    
	    Entity<Comida> data = Entity.entity(objComidaActualizar, MediaType.APPLICATION_JSON_TYPE);
	    objComida = target.request(MediaType.APPLICATION_JSON_TYPE).put(data, Comida.class);		
